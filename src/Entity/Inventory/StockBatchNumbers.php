 ...
 *   Create a draft stock transaction for a batch number.
 ...
 * Note:   In order to POST the following StockTransactionType [140, 141, 147, 148, 150, 151], it is required to have a Manufacturing solution.  Draft stock transaction for batch number is not supported for StockTransactionType [124, 125].  The batch number of part item (StockTransactionType = 165 or StockTransactionType = 166) can be linked via POST using the
 * ParentID property which is the same as the ID property for assembled item (StockTransactionType = 160 or StockTransactionType = 161).  The DraftStockTransactionID must be the same if multiple batch number is used for one pick order line.
 *
 * Example of JSON body:
 * When linking a batch number to a stock count line via POST:You can either create a new batch number for incoming quantity, or link an existing batch number for outgoing/incoming quantity.  StockCountLine is mandatory.  StockTransactionType should be Stock Count which is 195.  If StorageLocation is provided, this value will be validated. If StorageLocation is not provided, then the value for this field will be taken from StockCountLine.  The sign of Quantity stored will be set to be the same as the sign of quantity difference of the StockCountLine.  The DraftStockTransactionID provided should be the same as the DraftStockTransactionID of
 * another existing stock batch number record linked to the particular StockCountLine.When updating the quantity via PUT:  Quantity update is only supported for Stock count (StockTransactionType [195]) which is in draft.  The sign of Quantity stored will be set to be the same as the sign of quantity difference of the StockCountLine.
 *
 ...
 */
#[Exact\PageSize(60)]
#[Exact\Endpoint("/api/v1/{division}/inventory/StockBatchNumbers", "StockBatchNumbers")]
#[Exact\Method(HttpMethod::GET)]
#[Exact\Method(HttpMethod::POST)]
#[Exact\Method(HttpMethod::PUT)]
#[Exact\Method(HttpMethod::DELETE)]
class StockBatchNumbers extends DataSource
{
    ...
    #[EDM\Guid]
    #[Exact\Method(HttpMethod::GET)]
    public null|string $BatchNumberID = null;
    ...
    /**
     * ID representing a group of serial or batch number being reserved for use in a subsequent stock transaction
     *
     *
     * @var null|string Basic GUID type
     */
    ...
    /**
     * ID to link part item to assembled item (assembly order and pick order only)
     *
     *
     * @var null|string Basic GUID type
     */
    ...
    /**
     * ID of pick order entry in which this serial or batch number was used.
     *
     *
     * @var null|string Basic GUID type
     */
    ...
    /**
     * Quantity of this batch number entering or leaving inventory
     *
     *
    ...
    /**
     * ID of sales return entry in which this serial or batch number was used
     *
     *
     * @var null|string Basic GUID type
     */
